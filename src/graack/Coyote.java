...
//	private static final double REPRO_CHANCE_PER_REPRO_CYCLE = .325;
	private static final double REPRO_CHANCE_PER_REPRO_CYCLE = .01;
	...
	private boolean huntEnemyRabbits() {
		AbstractList<Agent> animals = smell(8, new SmellHandler() {
			@Override
			public boolean smellCondition(Agent smellee) {
				return smellee instanceof Herbivore
	...
	public boolean fightOrFlight() {
		AbstractList<Agent> carnivores = smell(15, new SmellHandler() {
			@Override
			public boolean smellCondition(Agent smellee) {
